...
public class HelloWorld : MonoBehaviour
{
	...
	void Awake()
	{
		...
		double gn = (double) _luaState.GetObject( "gn" );
		string gs = (string) _luaState.GetObject( "gs" );
		Debug.Log( string.Format( "gn: {0}, gs: '{1}'", gn, gs ) );

		LuaTable gt = _luaState.GetObject( "gt" ) as LuaTable;
		Debug.Log( string.Format( "gt.n: {0}, gt.s: '{1}'", gt["n"], gt["s"] ) );

		bool gtTB = (bool) _luaState.GetObject( "gt.t.b" );
		Debug.Log( string.Format( "gt.t.b: {0}", gtTB ) );

		LuaFunction gf = _luaState.GetObject( "gf" ) as LuaFunction;
